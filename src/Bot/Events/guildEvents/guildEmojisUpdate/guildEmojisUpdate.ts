...
export default async (payload: {
  ...
  const removedEmote = Array.from(client.emojis, ([, emoji]) => emoji).find((e) =>
    e.id ? !payload.emojis.get(BigInt(e.id)) : false,
  );
  const changedEmote = payload.emojis.find((e) =>
    e.id ? JSON.stringify(e) !== JSON.stringify(client.emojis.get(BigInt(e.id))) : false,
  );
  ...
  if (changedEmote && changedEmote.id) {
    const beforeEmote = client.emojis.get(BigInt(changedEmote.id));
