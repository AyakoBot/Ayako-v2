...
export function createProxyCache<T extends ProxyCacheTypes<boolean> = ProxyCacheTypes, B extends Bot = Bot>(
  rawBot: B,
  options: CreateProxyCacheOptions
): BotWithProxyCache<T, B> {
    ...
    set: async function (member: T["member"], fetch?: boolean | undefined): Promise<void> {
      ...
      if (options.cacheInMemory?.members) {
        if (options.cacheInMemory?.guilds) {
          if (member.guildId) {
            const guild =
              bot.cache.guilds.memory.get(member.guildId) ?? (await fetchers.fetchGuild(member.guildId, fetch, true));
  ...
  bot.cache.messages = {
    ...
    get: async function (
      ...
      if (options.cacheInMemory?.messages) {
        ...
        if (options.cacheInMemory?.guilds) {
          ...
          if (channelID) {
            ...
            } else console.error('cacheInMemory.messages" is enabled but a channel id was not found');
          }
        }
      ...
      if (options.fetchIfMissing?.messages && fetch) {
        console.error(
          new Error('"fetchIfMissing?.messages" is set to true but guild ID or channel ID was not provided')
        );
      }
    ...
    set: async function (message: T["message"]): Promise<void> {
      ...
      // If user wants non-memory cache, we cache it
      if (options.cacheOutsideMemory?.messages && options.setItem) await options.setItem("message", message);
    },
    delete: async function (id: BigString): Promise<void> {
      // Force id to bigint
      const messageID = BigInt(id);
