...
export default async (
 ...
 const auditUser =
  (webhook.owner ? await ch.getUser(webhook.owner.id) : undefined) ?? audit?.executor ?? undefined;
 ...
 ch.send({ id: channels, guildId: channel.guild.id }, { embeds: [embed], files }, undefined, 10000);
};
