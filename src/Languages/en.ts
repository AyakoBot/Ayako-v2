...
export default {
  ...
  events: {
    logs: {
      ...
      guild: {
        ...
        descMemberLeave: (user: CT.bEvalUser | Discord.User) =>
          `${getUser(user)}has left the Server`,
  ...
  features: {
    ...
    CREATOR_MONETIZABLE_DISABLED:
      '`Creator Monetizable disabled` `(Server currently does not meet the Requirements for Creator Monetization)`',
    DISCOVERABLE: '`Discoverable` `(Server can be discovered in the Server directory)`',
    DISCOVERABLE_DISABLED:
      '`Discorverable disabled` `(Server currently does not meet the Requirements for Discovery)`',
    ENABLED_DISCOVERABLE_BEFORE:
      '`Discorverable enabled before` `(Server had Discovery enabled before)`',
    ...
    ROLE_SUBSCRIPTIONS_AVAILABLE_FOR_PURCHASE:
      '`Role Subscriptions available for purchase` `(Members can buy Roles)`',
    ROLE_SUBSCRIPTIONS_ENABLED: '`Role Subscriptions enabled` `(Members can buy Roles)`',
    SEVEN_DAY_THREAD_ARCHIVE:
      '`Seven Day Thread Archive` `(Server has access to Seven Day Thread Archive Option)`',
    THREE_DAY_THREAD_ARCHIVE:
      '`Three Day Thread Archive` `(Server has access to Three Day Thread Archive Option)`',
    ...
    MEMBER_VERIFICATION_GATE_ENABLED:
      '`Member Verification Gate enabled` `(Server has enabled Membership Screening)`',
    ...
    VIP_REGIONS:
      '`VIP Voice Channel Regions` `(Server has access to VIP Voice Channels (384kbps))`',
