...
const showID = async (
  ...
  const settings = await client.ch
    .query(
      `SELECT * FROM ${client.customConstants.commands.settings.tableNames['multi-channels']} WHERE uniquetimestamp = $1;`,
      [parseInt(ID, 36)],
    )
...
const showAll = async (
  ...
  const settings = await client.ch
    .query(
      `SELECT * FROM ${client.customConstants.commands.settings.tableNames['multi-channels']} WHERE guildid = $1;`,
      [cmd.guild?.id],
    )
