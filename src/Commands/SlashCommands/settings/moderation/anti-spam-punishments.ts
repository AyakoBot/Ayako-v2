...
const showID = async (
  ...
  const settings = await client.ch
    .query(
      `SELECT * FROM ${client.customConstants.commands.settings.tableNames['anti-spam-punishments']} WHERE uniquetimestamp = $1 AND type = $2;`,
...
const showAll = async (
  ...
  const settings = await client.ch
    .query(
      `SELECT * FROM ${client.customConstants.commands.settings.tableNames['anti-spam-punishments']} WHERE guildid = $1 AND type = $2;`,
